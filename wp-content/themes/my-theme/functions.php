...
add_shortcode('ms_upcoming_events', function ($atts) {
    ...
    $allowed = [
        // Core
        'view','count','tz','compact','format','timeformat','pagination',
        'startdate','pastdays','limitdays','reverse',

        // Events/details
        'eventdesc','linktitles','nolink','eventdl',
        'hidetimes','hiderecurrence','hidecancelledevents','hideprivateevents','hidetentativeevents',
        'hidealldayindicator','color','category','eventlocaltime',

        // Layout/UX
        'ajax','monthnav','stickymonths','columnlabels','legendposition','combinemultiday',
        'extendmultiday','nomonthheaders','mapsource','maskinfo','subscribelink','whitetext',
        'htmltagtitle','htmltagmonth','htmltagdate','htmltagtime','htmltageventtitle','htmltageventdesc',
        'weeknumbers','guid','reload','skip','skipdomainerrors','skiprecurrence','solidcolors','tablebg',
...
add_action('admin_head', function () {
    ...
    <style>
        .column-notice_status { width: 110px; }
        .status {
            display:inline-block; padding:2px 8px; border-radius:999px; font-size:12px; line-height:1.5; color:#111;
            background:#e5e7eb;
        }
        .status--active  { background:#bbf7d0; }    /* green-ish */
        .status--expired { background:#fecaca; }    /* red-ish */
        .status--no-expiry { background:#fde68a; }  /* amber-ish */
    </style>
    <?php
});
