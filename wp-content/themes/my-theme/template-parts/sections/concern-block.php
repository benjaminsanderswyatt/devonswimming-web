...
$gf = static function (string $name) use ($version) { return get_field($name . $version); };
$sanitize_phone = static function ($p) { return preg_replace('/\s+/', '', (string)$p); };
...
$normalize_text = static function ($t) { return strtolower(trim((string)$t)); };
...
<section class="concern-block safeguard-block container" aria-labelledby="concern-heading<?php echo esc_attr($version); ?>">
  <?php if ($heading): ?>
